...
const SidebarContent = memo(({
  ...
  <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: '#ffffff', borderRight: '1px solid #f1f5f9' }}>
    <Toolbar sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', px: 2.5, py: 3 }}>
      <Box 
      ...
      >
        ...
        <KubeTypography weight="bold" variant="h6" sx={{ color: '#1e293b', fontSize: '1.25rem', letterSpacing: '-0.02em' }}>
          KubeStudy
        </KubeTypography>
      </Box>
      {isDesktop && (
        <IconButton onClick={handleDrawerToggle} size="small" sx={{ bgcolor: 'white', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', '&:hover': { bgcolor: '#f3f4f6' } }}>
          <ChevronLeftIcon fontSize="small" />
        </IconButton>
      )}
    </Toolbar>
    
    <Box sx={{ flexGrow: 1, overflowY: 'auto', px: 1, pb: 4 }}>
      <KubeTypography 
        variant="overline" 
        weight="bold"
        color="text.secondary"
        sx={{ px: 2, mt: 3, mb: 1, display: 'block', letterSpacing: 1.5, fontSize: '0.65rem', opacity: 0.8 }}
      >
        ...
        <>
          ...
          {modules.map((mod) => (
            <React.Fragment key={mod.id}>
              ...
              <Collapse in={activeModule && activeModule.id === mod.id} timeout="auto" unmountOnExit>
